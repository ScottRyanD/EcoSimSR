...
void Prey::reprod(Prey * friends, vector<Prey> * newPrey, Ecosystem * eco) {
	...
	if (bebe->getGender() == 1){ //bebe gender is female
		if (this->getGender() == 1)//female //MRE RandomGoodGene
		{
			if (getNbReprod() == 1){
		...
		{
			this->setEnergy(calculationEnergyNewParent(this->getEnergy(), energy, this->getStateBirth(), friends->getStateBirth()));			//-- Losing Energy for reproducing
			if (friends->getNbReprod() == 1){
				friends->setEnergy(calculationEnergyNewParent(friends->getEnergy(), energy, friends->getStateBirth(), this->getStateBirth()) - FirstPregnancyEneryWaste*getEnergy());			//-- Losing Energy for reproducing
			}
			else{
				friends->setEnergy(calculationEnergyNewParent(friends->getEnergy(), energy, friends->getStateBirth(), this->getStateBirth()));			//-- Losing Energy for reproducing
			}
		}
	}
	else{//bebe gender is male
		if (this->getGender() == 1)//female //MRE RandomGoodGene
		{
			if (getNbReprod() == 1){
				this->setEnergy(calculationEnergyNewParent(this->getEnergy(), energy, this->getStateBirth(), friends->getStateBirth()) - FirstPregnancyEneryWaste*getEnergy());			//-- Losing Energy for reproducing
