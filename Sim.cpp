...
float Sim::calculationEnergyAction(float energy, int nbArc, float speed, float strength, float persuasion, Ecosystem * eco){
	...
	if (phGenome.getGenomeSize() > 6){ // prey
		...
		float energyToSpend = ((1.2 * arcPenalty)  + ((strength*(pow((double)speed, 2))) / 4000.0) + pow((double)maxEnergy / 5.0, 0.75) + pow((double)vision * 2.5, 0.75) + (pow((double)maxSpeed * 3.0, 0.75)) + pow((double)defence * 5.0, 0.75) + pow((double)max(0.0, (double)(8 - repAge)), 2.3) + pow((double)persuasion * 5.0, 0.75));
		...
		float energyToSpend = ((1.2 * arcPenalty)  + ((strength*(pow((double)speed, 2))) / 7000) + pow((double)maxEnergy / 5.5, 0.75) + pow((double)vision * 2.0, 0.75) + pow((double)maxSpeed * 2.5, 0.75) + pow((double)max(0.0, (double)(7 - repAge)), 2.3) + pow((double)persuasion * 5.0, 0.75));
		e = energy - energyToSpend;
		if (getGender() == 0){
